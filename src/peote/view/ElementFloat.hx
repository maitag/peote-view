...
#if !macro
@:remove @:autoBuild(peote.view.ElementImplFloat.build()) // ONLY FOR TESTING ! (will be integrated into Element)
interface ElementFloat {}
class ElementImplFloat {}
#else
...
class ElementImplFloat
{
	...
	*/
	...
	{	//trace(f.name, type, val, getter, setter);
		if      ( hasMeta(f, "posX")  ) checkMetas(f, macro:Float, type, val, conf.posX, getter, setter);
		else if ( hasMeta(f, "posY")  ) checkMetas(f, macro:Float, type, val, conf.posY, getter, setter);
		else if ( hasMeta(f, "sizeX") ) checkMetas(f, macro:Float, type, val, conf.sizeX, getter, setter);
		else if ( hasMeta(f, "sizeY") ) checkMetas(f, macro:Float, type, val, conf.sizeY, getter, setter);
		else if ( hasMeta(f, "pivotX") ) checkMetas(f, macro:Float, type, val, conf.pivotX, getter, setter);
		else if ( hasMeta(f, "pivotY") ) checkMetas(f, macro:Float, type, val, conf.pivotY, getter, setter);
		...
		else if ( checkTexLayerMetas("texX",    f, macro:Float, type, val, conf.texXDefault, conf.texX, getter, setter) ) {}
		else if ( checkTexLayerMetas("texY",    f, macro:Float, type, val, conf.texYDefault, conf.texY, getter, setter) ) {}
		else if ( checkTexLayerMetas("texW",    f, macro:Float, type, val, conf.texWDefault, conf.texW, getter, setter) ) {}
		else if ( checkTexLayerMetas("texH",    f, macro:Float, type, val, conf.texHDefault, conf.texH, getter, setter) ) {}
		else if ( checkTexLayerMetas("texPosX", f, macro:Float, type, val, conf.texPosXDefault,  conf.texPosX, getter, setter) ) {}
		else if ( checkTexLayerMetas("texPosY", f, macro:Float, type, val, conf.texPosYDefault,  conf.texPosY, getter, setter) ) {}
		else if ( checkTexLayerMetas("texSizeX",f, macro:Float, type, val, conf.texSizeXDefault, conf.texSizeX, getter, setter) ) {}
		else if ( checkTexLayerMetas("texSizeY",f, macro:Float, type, val, conf.texSizeYDefault, conf.texSizeY, getter, setter) ) {}
	...
	public static function build()
	{
		...
		var buff_size_instanced:Int = Std.int(
			...
			+ 4 * (conf.posX.n  + conf.posY.n)
			+ 4 * (conf.sizeX.n + conf.sizeY.n)
			+ 4 * (conf.pivotX.n + conf.pivotY.n)
		);
		...
		for (c in conf.texX) buff_size_instanced += Std.int(c.n * 4);
		for (c in conf.texY) buff_size_instanced += Std.int(c.n * 4);
		for (c in conf.texW) buff_size_instanced += Std.int(c.n * 4);
		for (c in conf.texH) buff_size_instanced += Std.int(c.n * 4);
		for (c in conf.texPosX) buff_size_instanced += Std.int(c.n * 4);
		for (c in conf.texPosY) buff_size_instanced += Std.int(c.n * 4);
		for (c in conf.texSizeX) buff_size_instanced += Std.int(c.n * 4);
		for (c in conf.texSizeY) buff_size_instanced += Std.int(c.n * 4);
		...
		function writeBytesExpr(verts:Array<Array<Int>> = null):Array<Expr> {
			...
			for (j in 0...len)
			{
				...
				// POS
				if (conf.posX.isAnim && conf.posX.isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.posX.name+"Start"}) ); i+=4; }
				if (!conf.posX.isAnim && conf.posX.isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.posX.name }) ); i+=4; }
				if (conf.posY.isAnim && conf.posY.isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.posY.name+"Start"}) ); i+=4; }
				if (!conf.posY.isAnim && conf.posY.isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.posY.name }) ); i+=4; }
				if (conf.posX.isAnim && conf.posX.isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.posX.name+"End"}) ); i+=4; }
				if (conf.posY.isAnim && conf.posY.isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.posY.name+"End"}) ); i+=4; }
				// SIZE
				if (conf.sizeX.isAnim && conf.sizeX.isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.sizeX.name+"Start"}) ); i+=4; }
				if (!conf.sizeX.isAnim && conf.sizeX.isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.sizeX.name}) ); i+=4; }
				if (conf.sizeY.isAnim && conf.sizeY.isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.sizeY.name+"Start"}) ); i+=4; }
				if (!conf.sizeY.isAnim && conf.sizeY.isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.sizeY.name}) ); i+=4; }
				if (conf.sizeX.isAnim && conf.sizeX.isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.sizeX.name+"End"}) ); i+=4; }
				if (conf.sizeY.isAnim && conf.sizeY.isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.sizeY.name+"End"}) ); i+=4; }
				// PIVOT
				if (conf.pivotX.isAnim && conf.pivotX.isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.pivotX.name+"Start"}) ); i+=4; }
				if (!conf.pivotX.isAnim && conf.pivotX.isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.pivotX.name}) ); i+=4; }
				if (conf.pivotY.isAnim && conf.pivotY.isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.pivotY.name+"Start"}) ); i+=4; }
				...
				for (k in 0...conf.texX.length) {
					...
					if (!conf.texX[k].isAnim && conf.texX[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texX[k].name}) ); i+=4; }
					if (conf.texX[k].isAnim && conf.texX[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texX[k].name+"End"}) ); i+=4; }
				}
				// TEXY
				for (k in 0...conf.texY.length) {
					if (conf.texY[k].isAnim && conf.texY[k].isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texY[k].name+"Start"}) ); i+=4; }
					if (!conf.texY[k].isAnim && conf.texY[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texY[k].name}) ); i+=4; }
					if (conf.texY[k].isAnim && conf.texY[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texY[k].name+"End"}) ); i+=4; }
				}
				// TEXW
				for (k in 0...conf.texW.length) {
					if (conf.texW[k].isAnim && conf.texW[k].isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texW[k].name+"Start"}) ); i+=4; }
					if (!conf.texW[k].isAnim && conf.texW[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texW[k].name}) ); i+=4; }
					if (conf.texW[k].isAnim && conf.texW[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texW[k].name+"End"}) ); i+=4; }
				}
				// TEXH
				for (k in 0...conf.texH.length) {
					if (conf.texH[k].isAnim && conf.texH[k].isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texH[k].name+"Start"}) ); i+=4; }
					if (!conf.texH[k].isAnim && conf.texH[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texH[k].name}) ); i+=4; }
					if (conf.texH[k].isAnim && conf.texH[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texH[k].name+"End"}) ); i+=4; }
				}
				// TEXPOSX
				for (k in 0...conf.texPosX.length) {
					if (conf.texPosX[k].isAnim && conf.texPosX[k].isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texPosX[k].name+"Start"}) ); i+=4; }
					if (!conf.texPosX[k].isAnim && conf.texPosX[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texPosX[k].name}) ); i+=4; }
					if (conf.texPosX[k].isAnim && conf.texPosX[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texPosX[k].name+"End"}) ); i+=4; }
				}
				// TEXPOSY
				for (k in 0...conf.texPosY.length) {
					if (conf.texPosY[k].isAnim && conf.texPosY[k].isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texPosY[k].name+"Start"}) ); i+=4; }
					if (!conf.texPosY[k].isAnim && conf.texPosY[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texPosY[k].name}) ); i+=4; }
					if (conf.texPosY[k].isAnim && conf.texPosY[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texPosY[k].name+"End"}) ); i+=4; }
				}
				// TEXSIZEX
				for (k in 0...conf.texSizeX.length) {
					if (conf.texSizeX[k].isAnim && conf.texSizeX[k].isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texSizeX[k].name+"Start"}) ); i+=4; }
					if (!conf.texSizeX[k].isAnim && conf.texSizeX[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texSizeX[k].name}) ); i+=4; }
					if (conf.texSizeX[k].isAnim && conf.texSizeX[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texSizeX[k].name+"End"}) ); i+=4; }
				}
				// TEXSIZEY
				for (k in 0...conf.texSizeY.length) {
					if (conf.texSizeY[k].isAnim && conf.texSizeY[k].isStart) { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texSizeY[k].name+"Start"}) ); i+=4; }
					if (!conf.texSizeY[k].isAnim && conf.texSizeY[k].isStart){ exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texSizeY[k].name}) ); i+=4; }
					if (conf.texSizeY[k].isAnim && conf.texSizeY[k].isEnd)   { exprBlock.push( macro bytes.setFloat(bytePos + $v{i}, $i{conf.texSizeY[k].name+"End"}) ); i+=4; }
				}
				// -------------- setUInt16 ------------------------------
		...
		function enableVertexAttribExpr(isInstanced:Bool=false):Array<Expr> {
			...
			if (n > 0 ) {
				...
				exprBlock.push( macro gl.vertexAttribPointer(aPOS, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			if (n > 0 ) {
				...
				exprBlock.push( macro gl.vertexAttribPointer(aSIZE, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			if (n > 0 ) {
				...
				exprBlock.push( macro gl.vertexAttribPointer(aPIVOT, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texX.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXX"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texY.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXY"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texW.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXW"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texH.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXH"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texPosX.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXPOSX"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texPosY.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXPOSY"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texSizeX.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXSIZEX"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
			...
			for (k in 0...conf.texSizeY.length) {
				...
				if (n > 0 ) {
					...
					exprBlock.push( macro gl.vertexAttribPointer($i{"aTEXSIZEY"+k}, $v{n}, gl.FLOAT, false, $v{stride}, $v{i} ) ); i += n * 4;
